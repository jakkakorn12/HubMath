...
export default function QuizSetManager({
  ...
  for (const s of sets) (byDifficulty[s.difficulty] ??= []).push(s);
    ...
    <div className="space-y-6">
      ...
      {sets.length === 0 ? (
        ...
        DIFFICULTY_ORDER.filter((d) => byDifficulty[d]?.length).map((d) => (
          <div key={d} className="bg-white rounded-card border-[0.5px] border-border p-5">
            <h3 className="text-sm font-semibold text-ink-muted mb-3">ระดับ{DIFFICULTY_LABEL[d]}</h3>
